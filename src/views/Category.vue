  ...
  <v-container class="mt-4">
    <v-row>
      ...
      <v-btn color="primary" @click="dialog=!dialog">
        ...
        <span class="ml-2">Yangi kategoriyalar</span>
      </v-btn>
    </v-row>
    <v-row>
      <v-col cols="12">
        <v-data-table :headers="headers" :items="categories" :items-per-page="10" class="elevation-1">
        ...
        <v-card-text>
          <v-container>
            <v-row>
              ...
              <v-col cols="12" sm="12">
                <v-checkbox v-model="category.hide" label="`Vaqtincha mavjud emas:`"></v-checkbox>
              </v-col>
            </v-row>
          </v-container>
        </v-card-text>
  ...
  export default {
    ...
    methods: {
      save(){
        ...
        this.category={}
        this.toggle=false
        axios.post('http://localhost:3000/category'+ this.category.id,this.category ).then(response => {
      ...
      add() {
        axios.post('http://localhost:3000/category', this.category).then(response => {
